...
func TestGuildServerConfigRoundTripUsesConditionalRevision(t *testing.T) {
	...
	config := domain.GuildServerConfig{GuildID: "guild-1", Revision: 1, ObjectKey: "guilds/guild-1/server-config/revisions/000001-" + strings.Repeat("a", 64) + "/server.cfg", Filename: "private.cfg", SHA256: strings.Repeat("a", 64), SizeBytes: 20, UploadedBy: "admin-1", UpdatedAt: now}
	client := &fakeAPI{}
