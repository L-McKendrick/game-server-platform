...
locals {
  ...
  sleep_wake_definitions = {
    ...
    WakeSession = jsonencode({ StartAt = "Dispatch", States = {
      Dispatch                  = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "dispatch", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id" } }, ResultPath = "$.stage", Catch = [{ ErrorEquals = ["States.ALL"], ResultPath = "$.failure", Next = "DispatchRollback" }], Next = "WaitForEC2" }
      ...
      ManagedNodeReady           = { Type = "Choice", Choices = [{ Variable = "$.managed.result.managed", BooleanEquals = true, Next = "DispatchMods" }], Default = "IncrementManagedAttempts" }
      ...
      DispatchMods               = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "dispatch_mods", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id" } }, ResultSelector = { "result.$" = "$.Payload" }, ResultPath = "$.mods", Catch = [{ ErrorEquals = ["States.ALL"], ResultPath = "$.failure", Next = "DispatchRollback" }], Next = "ModsDispatchResult" }
      ModsDispatchResult         = { Type = "Choice", Choices = [{ Variable = "$.mods.result.succeeded", BooleanEquals = true, Next = "DispatchHealth" }], Default = "InitializeModsAttempts" }
      ...
      ObserveMods                = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "observe_mods", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id", "command_id.$" = "$.mods.result.command_id" } }, ResultSelector = { "result.$" = "$.Payload" }, ResultPath = "$.mods", Catch = [{ ErrorEquals = ["States.ALL"], ResultPath = "$.failure", Next = "DispatchRollback" }], Next = "ModsResult" }
      ...
      DispatchHealth             = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "dispatch_health", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id" } }, ResultSelector = { "result.$" = "$.Payload" }, ResultPath = "$.health", Catch = [{ ErrorEquals = ["States.ALL"], ResultPath = "$.failure", Next = "DispatchRollback" }], Next = "WaitForHealth" }
      ...
      Complete                   = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "complete", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id" } }, End = true }
      DispatchRollback           = { Type = "Task", Resource = "arn:aws:states:::lambda:invoke", Parameters = { FunctionName = aws_lambda_function.sleepwake_worker.function_name, Payload = { action = "dispatch_rollback", "session_id.$" = "$.session_id", "workflow_id.$" = "$.workflow_id", "correlation_id.$" = "$.correlation_id" } }, ResultSelector = { "result.$" = "$.Payload" }, ResultPath = "$.rollback", Catch = [{ ErrorEquals = ["States.ALL"], ResultPath = "$.rollback_failure", Next = "Fail" }], Next = "RollbackDispatched" }
