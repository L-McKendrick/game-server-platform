...
1. Prepare and monitor the controlled beta using the completed Phase 19.3 managed-host isolation boundary.
...
18. **Discord Lifecycle UX and Restart — Done**
    ...
    - **18.2** [x] Add a simple, robust `/rb restart` operation for active game servers.
      - **18.2.1** [x] Define `/rb restart` as an owner/admin-authorized operation for a stable running or idle session that uses the existing session workflow lock and rejects conflicting or inapplicable lifecycle states. Restart immediately without confirmation even when players are connected; restart only the game server, leaving TeamSpeak and the EC2 host running.
      - **18.2.2** [x] Reuse the bounded managed-host content and service-control paths to apply pending mod, server-mod, and game-server settings changes, restart Arma, and verify health. Reuse installation of genuinely pending mission files when present; already installed missions need no restart-specific handling. Do not add mission selection handling or rollback scope; preserve existing shared-path safeguards and report failures truthfully.
      - **18.2.3** [x] Expose concise restart progress and actionable failure state through existing private status and public-card update contracts without adding a new persistent service or overcomplicated orchestration.
      - **18.2.4** [x] Add focused authorization, state-drift, workflow-conflict, pending-change, no-change, restart failure, health-check, replay, and command-registration coverage.
    - **18.3** [x] Complete creation readiness automation and notification behavior.
      - **18.3.1** [x] Add an off-by-default `Notify when ready?` option to `/rb create`, persist the owner preference and invoking channel needed for delivery, and suppress all unrelated mentions.
    ...
    - **18.6** [x] Correct lifecycle controls discovered during live setup acceptance.
    ...
    - **18.9** [x] Remove inert controls from stable offline public cards.
...
19. **Production Guardrails and Managed-Host Isolation — In Progress**
    - Started after Phase 20.2 by explicit user direction on
      ...
      an accepted supervised-development risk only. Phase 19.3 is the next
      priority and must close before beta testing release, production, or
      multi-tenant use. Do not rotate the Steam authorization cache without
      evidence of exposure.
    - Keep Phase 19.3 lean: reuse existing workers, workflow records, queues,
      SSM dispatch, and reliability handling wherever they satisfy isolation.
      Introduce new services, persistence, refresh loops, or large-object copies
      only for demonstrated requirements; keep Discord usability unchanged.
    ...
    - **19.3** [x] Close SEC-20-01 before beta testing release by removing standing managed-host S3 access and mediating exact authorized reads and bounded uploads. Cover cross-session disclosure and tampering, unrelated guild configuration, and durable-output integrity; retain the separately mediated Phase 19.1 Steam boundary. Review findings are stored in this chat's external `phase-19-3-beta-host-isolation-context.md`, not in the project.
      - **19.3.1** [x] Finalize the reviewed host-access inventory and SDK-independent operation/attempt contracts for bootstrap scripts, accepted missions, client/server presets, guild config snapshots, Workshop outputs, progress, logs, and archives. Contract: `docs/phase-19-host-object-access.md`, domain scoped-object/expiry rules, and bounded ephemeral port codecs. Offline pinned-SDK evidence proves exact length/type/checksum/create-only signing and exact-key POST byte bounds; 1,000-object manifests use a compact SSM reference. Use existing observation/reliability paths for credential-clipped refresh, pinned staged source versions for promotion, and a bounded archive prepare/upload handoff. Live enforcement/renewal remain 19.3.8 gates. Distinguish trusted session/workflow/instance/purpose authorization from reusable bearer URL enforcement; no native single-use or host-identity binding is claimed.
      - **19.3.2** [x] Implement shared trusted-worker access issuance using existing conditional workflow records where sufficient; add attempt persistence only for required recovery or independent live-copy operations. Strongly revalidate session, applicable workflow or accepted live-copy operation, exact instance and immutable tags, purpose, and content snapshot before issuance, refresh, verification, or promotion. Issue only exact-object capabilities with enforced constraints and bounded expiry; keep raw URLs out of durable records and ordinary logs. Add wrong-identity, stale-state, expiry, dispatch-ambiguity, replay, refresh, and cleanup coverage without introducing a public arbitrary-key signing endpoint. Complete: shared object/manifest issuance, exact running-instance authority, credential-clipped SDK signing, encrypted versioned exchange, conditional recovery metadata, dispatch/replay/refresh, bounded live copy and persisted-owner expired-attempt cleanup. Focused security/recovery tests and vet pass; runtime wiring and live acceptance remain later tasks.
      - **19.3.3** [x] Migrate bootstrap-script and accepted-input delivery in bootstrap, wake, restart, restore bootstrap, and live mission copy to compact capability manifests and bounded HTTPS downloads. Complete in source: full deployed script digest verification; mission/config checksums and preset revisions/rollback; bounded pinned legacy input verification; stable command/context recovery; observation-driven generation renewal and bounded expiry retry; root-only cleanup and 1,000-mission command-budget coverage. No asset credentials added to user data. Focused tests/vet and Terraform validation pass. Deploy only with completed output/IAM/runtime cutover in 19.3.4–7; live acceptance remains 19.3.8.
      - **19.3.4** [x] Migrate Workshop mission, resolution and sync-result publication to finite exact attempt staging slots with resolved-size POST bounds and bounded streamed HTTPS uploads. Trusted successful-command observations validate bytes/digests, approved metadata/provenance, complete item sets and content revisions before immutable full-set source pins and create-only durable publication. Canonical sorted resolutions preserve full/pending intent and removals; rollback inventory/results, workflow-scoped checkpoints and pinned partial-publication recovery are covered. Reuses existing workers/records/storage; no whole-PBO buffering or new service. Focused application/worker/storage/helper/shell/SDK tests and vet pass. Offline source completion only; legacy TSV reconciliation and atomic IAM/runtime cutover remain 19.3.7, live acceptance 19.3.8.
      - **19.3.5** [x] Migrate archive/restore to constrained streamed uploads and exact verified archive reads. Existing runner observations orchestrate actual-size/checksum preparation, immutable manifest issuance and create-only exact-key PUT; trusted whole-object checksum/type/length/non-null-version HEAD gates replay, manifest completion and infrastructure destruction. Restore pins accepted own-session archive versions and preserves mount/exact-device, expansion/extraction, auth scrub and health/bootstrap safeguards. Shared owned-command recovery/renewal and bounded same-object expiry retry preserve original deadlines and retained-resource behavior. No new service, table, queue or whole-archive Lambda buffering. Focused app/adapter/worker tests, vet and Terraform validation pass. Offline source completion only; progress/diagnostics 19.3.6, atomic IAM/runtime cutover 19.3.7 and live acceptance 19.3.8 remain.
      - **19.3.6** [x] Migrate advisory progress to exact attempt POST slots and owned-scope readers, preserving immutable renewal inventory and prompt sampler shutdown. Retire SSM OutputS3 in bootstrap/archive/restore; bounded native SSM invocation output retains sanitized failure diagnostics and compact results without a new log service. Signed mandatory staging tags drive three-day current/noncurrent version expiry and empty delete-marker removal. Existing reliability/termination workers use strong paginated session-partition queries, persisted owner/expiry checks and repeatable version sweeps; metadata remains for replay protection, routine old-prefix sweeps stop after four days, and lifecycle covers late uploads. Accepted Workshop outputs exclude telemetry; existing reset/termination all-version cleanup and reset drift refusal remain intact. Focused executable/policy/app/storage/SSM/cleanup tests, affected worker build/vet and Terraform validation pass. Offline source completion only; issuer tagging grants and host IAM/runtime cutover remain 19.3.7, live admission 19.3.8.
      - **19.3.7** [x] Complete in source: remove both managed-host asset inline policies and audit the sole reviewed SSM attachment and TLS-only bucket policy. Existing issuers receive purpose-prefix pinned reads/tagged writes, verified publication, archive and conditional-attempt grants; reliability gets staging-only version cleanup, termination gets session queries. All issuers share scoped-host-access-v1, normalized uploaded-script digests and explicit policy dependencies. Quiesced rollout, retained-host/legacy TSV migration and admission-closing rollback are documented; security docs reconciled. Ninety native-provider offline aggregate permission checks and quota margins, seven deployment-verifier positives/four drift negatives, Go 1.26.5 coverage/vet/build/final regressions/packaging and Terraform 1.15.8 fmt/validation pass. The later 19.3.8 deployment, live acceptance, cleanup and Ubuntu CI race gate passed.
      - **19.3.8** [x] Through separately approved fresh plans and disposable live exercises, verify the full vanilla/modded/TeamSpeak lifecycle, live mission copy, Workshop sync, long-operation expiry recovery, and failure diagnostics with final permissions. Use one host's actual credentials to prove read/write denial against another session in the same guild and another guild, unrelated config, archives, progress/results/logs, bootstrap revisions, and Steam exchanges; verify secret/lease denial. Record exact revisions, constrained-capability negative checks, cleanup, and an owned beta admission decision. Do not admit beta testers or close SEC-20-01 on offline tests or single-session success alone.
        - Final acceptance (2026-09-18): test-67 proved modded/TeamSpeak renewal, lifecycle, live-copy, Workshop and actual-host isolation; test-69/test-70 exposed and verified archive-upload and terminal Steam cleanup corrections; test-71 completed vanilla/TeamSpeak archive, guarded destruction and replacement-host restore. The final ArchiveSession correction is deployed, all four sessions are deleted, test-71 has no tagged EC2/EBS or session-owned S3 versions, completed Steam temporary versions are lifecycle-managed, and the Ubuntu CI race gate passed. The platform owner approved controlled beta preparation; production hardening remains in Phase 20.
