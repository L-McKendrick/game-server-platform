...
func run(ctx context.Context) error {
	...
	commands := make([]json.RawMessage, 0, len(commandDefinitionPaths))
	for _, definitionPath := range commandDefinitionPaths {
	...
	request.Header.Set("User-Agent", "game-server-platform/phase-4")
	...
	fmt.Printf("Registered development /session and /admin commands for guild %s.\n", guildID)
	return nil
}
