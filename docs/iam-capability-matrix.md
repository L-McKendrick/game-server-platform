...
| Bootstrap worker | Conditional metadata writes; tagged SSM commands; bounded progress/manifest reads; broker an exact Steam exchange; enqueue notifications | No EC2 creation/destruction, unrelated secret reads, or arbitrary S3 objects |
...
| Reliability worker | Reconcile metadata/workflows; observe SSM/EC2/S3; tag/quarantine or clean tagged orphan compute; inspect/redrive declared DLQs; broker an exact Steam exchange for recovered content sync; notifications | No S3 deletion outside exchange cleanup, security-group deletion, resource creation, or unrelated secrets |
...
| Managed game instance | SSM core; session input/archive reads; session log/archive/progress/result writes; exact expiring Steam-exchange HTTPS capabilities only during an authorized operation | No EC2/IAM/queue/workflow, Secrets Manager, or Steam-lease DynamoDB APIs; cross-session S3 scope is accepted only for supervised development until Phase 19.3 closes it |
